...
def _dinov2_repo_dir() -> str:
    ...
    if not os.path.isdir(repo_dir):
        raise FileNotFoundError(
            f"Local DINOv2 repo cache not found: {repo_dir}. "
            "Expected a cached torch.hub checkout."
        )
    return repo_dir


def _dinov2_weights_path(vit_type: str) -> str:
    weights_dir = os.environ.get(
        "DINOV2_CHECKPOINT_DIR",
        os.path.join(torch.hub.get_dir(), "checkpoints"),
    )
    weights_path = os.path.join(weights_dir, f"dinov2_{vit_type}14_pretrain.pth")
    if not os.path.isfile(weights_path):
        raise FileNotFoundError(
            f"Local DINOv2 checkpoint not found: {weights_path}. "
            "Expected a cached torch.hub checkpoint."
        )
    return weights_path
...
class MultiViewUniMatch(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        repo_dir = _dinov2_repo_dir()
        weights_path = _dinov2_weights_path(encoder)
