...
from ..dataset.shims.camera_noise_shim import (
    CameraNoiseCfg,
    apply_camera_noise_shim,
)
...
class ModelWrapper(LightningModule):
    ...
    def __init__(
        ...
        if self.test_cfg.compute_scores:
            ...
            self.test_scene_outputs = []

    def _compute_metrics_chunked(
        self,
        ground_truth: Float[Tensor, "view channel height width"],
        predicted: Float[Tensor, "view channel height width"],
    ) -> tuple[float, float, float]:
        chunk_size = max(1, self.test_cfg.metric_chunk_size)
        psnr_scores = []
        ssim_scores = []
        lpips_scores = []
        for start in range(0, ground_truth.shape[0], chunk_size):
            ...
            psnr_scores.append(compute_psnr(gt_chunk, pred_chunk))
            ssim_scores.append(compute_ssim(gt_chunk, pred_chunk))
            lpips_scores.append(compute_lpips(gt_chunk, pred_chunk))

        psnr_value = torch.cat(psnr_scores).mean().item()
        ssim_value = torch.cat(ssim_scores).mean().item()
        lpips_value = torch.cat(lpips_scores).mean().item()
    ...
    def test_step(self, batch, batch_idx):
        if batch_idx == 0 and self.test_cfg.compute_scores:
            torch.cuda.reset_peak_memory_stats()
        batch: BatchedExample = self.data_shim(batch)
        batch = apply_camera_noise_shim(batch, self.test_cfg.camera_noise)
        ...
        if self.test_cfg.compute_scores:
            ...
            if not self.train_cfg.forward_depth_only:
                rgb = images_prob
                psnr_value, ssim_value, lpips_value = self._compute_metrics_chunked(
                    rgb_gt,
                    rgb,
                )
                ...
                self.test_step_outputs[f"psnr"].append(psnr_value)
                self.test_step_outputs[f"ssim"].append(ssim_value)
                self.test_step_outputs[f"lpips"].append(lpips_value)
                self.test_scene_outputs.append(
                    {
                        "scene": scene,
                        "psnr": psnr_value,
                        "ssim": ssim_value,
