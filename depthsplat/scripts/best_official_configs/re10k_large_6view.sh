#!/usr/bin/env bash
set -euo pipefail
# RE10K large 6-view: uses base re10k+dl3dv checkpoint with base configuration.
# Named "large" for consistency, but uses base model checkpoint and config.

PROJECT_ROOT="${PROJECT_ROOT:-/root/depthsplat}"
DATA_ROOT="${DATA_ROOT:-/root/depthsplat/datasets/re10k_720p}"
OUTPUT_DIR="${OUTPUT_DIR:-/root/autodl-tmp/outputs/depthsplat_best_configs/re10k_large_6view}"
...
"$PYTHON_BIN" -m src.main +experiment=dl3dv \
  ...
  checkpointing.pretrained_model="${CKPT_PATH:-pretrained/depthsplat-gs-base-re10kdl3dv-448x768-randview2-6-f8ddd845.pth}" \
  mode=test \
  dataset/view_sampler=evaluation \
  dataset.view_sampler.num_context_views=6 \
  dataset.view_sampler.index_path="${EVAL_INDEX:-assets/re10k_ctx_6v_video.json}" \
