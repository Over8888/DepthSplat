#!/usr/bin/env bash
set -euo pipefail
# Official DepthSplat best RE10K config: small model, 2-view, 256x256.

PROJECT_ROOT="${PROJECT_ROOT:-/root/depthsplat}"
DATA_ROOT="${DATA_ROOT:-/root/autodl-tmp/RealEstate10K}"
OUTPUT_DIR="${OUTPUT_DIR:-/root/autodl-tmp/outputs/depthsplat_best_configs/re10k_small_2view}"
...
"$PYTHON_BIN" -m src.main +experiment=re10k \
  ...
  checkpointing.pretrained_model="${CKPT_PATH:-pretrained/depthsplat-gs-small-re10k-256x256-view2-cfeab6b1.pth}" \
  mode=test \
