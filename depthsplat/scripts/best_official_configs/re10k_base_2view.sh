#!/usr/bin/env bash
set -euo pipefail
# Official DepthSplat best RE10K config: base model, 2-view, 256x256.

PROJECT_ROOT="${PROJECT_ROOT:-/root/depthsplat}"
DATA_ROOT="${DATA_ROOT:-/root/autodl-tmp/RealEstate10K}"
OUTPUT_DIR="${OUTPUT_DIR:-/root/autodl-tmp/outputs/depthsplat_best_configs/re10k_base_2view}"
...
"$PYTHON_BIN" -m src.main +experiment=re10k \
  ...
  checkpointing.pretrained_model="${CKPT_PATH:-pretrained/depthsplat-gs-base-re10k-256x256-view2-ca7b6795.pth}" \
  mode=test \
