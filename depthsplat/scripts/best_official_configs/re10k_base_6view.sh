#!/usr/bin/env bash
set -euo pipefail
# Official DepthSplat Table 7 DL3DV config: base model, 6-view, 512x960 inference.
...
OUTPUT_DIR="${OUTPUT_DIR:-/root/autodl-tmp/outputs/depthsplat_best_configs/re10k_base_6view}"
...
"$PYTHON_BIN" -m src.main +experiment=dl3dv \
  ...
  dataset.view_sampler.index_path="${EVAL_INDEX:-assets/dl3dv_start_0_distance_50_ctx_6v_video_0_50.json}" \
